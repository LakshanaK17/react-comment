...
export const commentSlice = createSlice({
  ...
  reducers: {
    ...
    addReplyToCommentSuccess: (state, action) => {
      ...
      if (commentIndex !== -1) {
        state.comments[commentIndex].replies.push(reply); // Assuming replies is an array in the comment object
      }
    },
...
export const addReplyToComment = ({ id, content }) => async dispatch => {
  try {
    ...
    dispatch(addReplyToCommentSuccess({ id, reply: response.data }));
    return response.data; // Return the newly added reply from the backend
  } catch (error) {
    dispatch(setError(error.message));
...
export const { setComments, addComment, editComment, deleteComment, incrementLikes, setError, addReplyToCommentSuccess } = commentSlice.actions;
